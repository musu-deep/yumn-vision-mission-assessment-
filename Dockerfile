...
RUN set -eux; \
    ...
    python -c "from pathlib import Path; p=Path('frontend/index.html'); a=Path('frontend/database-primary.js').read_text(encoding='utf-8'); b=Path('frontend/secure-public.js').read_text(encoding='utf-8'); s=p.read_text(encoding='utf-8'); p.write_text(s.replace('</body>', '<script>\\n'+a+'\\n</script>\\n<script>\\n'+b+'\\n</script>\\n</body>', 1), encoding='utf-8')"; \
